...
bool is_valid_object_status(uint64_t status) {
    return status == kNormalStatus || status == kEndOfGroupStatus ||
           status == kEndOfTrackStatus;
}

ParseStatus read_varint_at(
    const ByteBuffer& bytes, size_t& offset, uint64_t& value, std::string& error) {
...
ParseStatus read_properties_at(
...
private:
    ParseResult parse_header() {
        ...
        if (!route_) {
            if (plane_.unknown_alias_policy() == UnknownAliasPolicy::Error) {
                return ParseResult{
                    ParseStatus::Error,
                    "subgroup stream referenced unknown track alias " +
                        std::to_string(alias_)};
            }
    ...
    ParseResult parse_object() {
        ...
        if (payload_length == 0) {
            ...
            if (!is_valid_object_status(status_code) ||
                (status_code != kNormalStatus && !properties.empty())) {
        ...
        if (last_object_id_) {
            if (*last_object_id_ == std::numeric_limits<uint64_t>::max() ||
                object_delta > std::numeric_limits<uint64_t>::max() -
                                   (*last_object_id_ + 1)) {
    ...
    void erase_prefix(size_t offset) {
        buffer_.erase(
            buffer_.begin(), buffer_.begin() + static_cast<std::ptrdiff_t>(offset));
    }
...
bool TrackReceiveValidation::validate(const Object& object, std::string& error) const {
    ...
    if (final_in_group != final_object_in_group_.end() &&
        object.object_id > final_in_group->second) {
    ...
    if (final_object_in_track_ &&
        (object.group_id > final_object_in_track_->group ||
         (object.group_id == final_object_in_track_->group &&
          object.object_id > final_object_in_track_->object))) {
...
void TrackReceiveValidation::mark_final_object_in_group(
    GroupId group_id, ObjectId object_id) {
    final_object_in_group_[group_id] = object_id;
}

void TrackReceiveValidation::mark_final_object_in_track(Location location) {
    final_object_in_track_ = location;
}
...
DataPlane::DataPlane(
    ...
    : config_(std::move(config)),
      protocol_error_callback_(std::move(protocol_error)),
...
void DataPlane::on_datagram(ByteBuffer bytes) {
    deliver_datagram(std::move(bytes), true);
}

void DataPlane::start_subgroup_stream(
    std::shared_ptr<TransportStream> stream,
    ByteBuffer initial_bytes,
    bool fin) {
    auto receiver = std::make_shared<DataStreamReceiver>(*this, stream);
    stream->on_bytes([receiver](ByteBuffer bytes, bool bytes_fin) mutable {
        receiver->feed(std::move(bytes), bytes_fin);
    });
    stream->on_peer_send_aborted([receiver](uint64_t error_code) {
        receiver->aborted(error_code);
    });
    receiver->feed(std::move(initial_bytes), fin);
}

UnknownAliasPolicy DataPlane::unknown_alias_policy() const {
    return config_.unknown_alias_policy;
}
...
void DataPlane::deliver_datagram(ByteBuffer bytes, bool allow_buffer) {
    ...
    if ((type > 0x0f && (type < 0x20 || type > 0x2f)) ||
        ((type & 0x20) != 0 && (type & 0x02) != 0)) {
    ...
    if ((type & 0x04) == 0 &&
        read_varint_at(bytes, offset, object_id, error) != ParseStatus::Done) {
    ...
    if ((type & 0x08) == 0 &&
        read_byte_at(bytes, offset, publisher_priority) != ParseStatus::Done) {
    ...
    if ((type & 0x20) != 0) {
        ...
        if (read_varint_at(bytes, offset, status_code, error) != ParseStatus::Done ||
            !is_valid_object_status(status_code) ||
            (status_code != kNormalStatus && !properties.empty()) ||
            offset != bytes.size()) {
...
void DataPlane::deliver(std::shared_ptr<ReceiveRoute> route, Object object) {
    ...
    if (object.status && *object.status == kEndOfTrackStatus) {
        route->validation->mark_final_object_in_track(
