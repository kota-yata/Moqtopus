    ...
    private:
            ...
            std::shared_ptr<std::promise<RequestOk>> promise)
        {
            ...
            try
            {
                ...
                auto sender = [stream](ByteBuffer bytes)
                { return stream->send(std::move(bytes)); };
