...
TransportStream::TransportStream(MsQuicTransportAdapter &adapter, HQUIC handle, bool unidirectional)
    : adapter_(adapter), handle_(handle), unidirectional_(unidirectional) {}

TransportStream::~TransportStream() = default;

bool TransportStream::unidirectional() const { return unidirectional_; }

uint64_t TransportStream::id() const { return id_; }

void TransportStream::set_id(uint64_t id) { id_ = id; }

void TransportStream::on_bytes(BytesCallback callback) { bytes_callback_ = std::move(callback); }
...
void TransportStream::on_shutdown(ShutdownCallback callback) { shutdown_callback_ = std::move(callback); }
...
QUIC_STATUS QUIC_API TransportStream::stream_callback(HQUIC stream, void *context, QUIC_STREAM_EVENT *event) {
...
std::shared_ptr<TransportStream> MsQuicTransportAdapter::open_stream(bool unidirectional) {
  ...
  api_->SetCallbackHandler(handle, reinterpret_cast<void *>(TransportStream::stream_callback), stream.get());
  {
    std::lock_guard<std::mutex> lock(streams_mutex_);
...
QUIC_STATUS MsQuicTransportAdapter::handle_connection_event(HQUIC connection, QUIC_CONNECTION_EVENT *event) {
  ...
  case QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED: {
    ...
    auto stream =
        std::shared_ptr<TransportStream>(new TransportStream(*this, event->PEER_STREAM_STARTED.Stream, unidirectional));
    api_->SetCallbackHandler(event->PEER_STREAM_STARTED.Stream,
                             reinterpret_cast<void *>(TransportStream::stream_callback), stream.get());
    {
      std::lock_guard<std::mutex> lock(streams_mutex_);
      streams_.emplace(stream.get(), stream);
    }
